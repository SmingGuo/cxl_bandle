...
# They favor higher throughput while keeping mean/p99 latency below 2x of the
# earlier conservative defaults. Override with
# BANDLE_ADMISSION_WINDOW/BANDLE_ADMISSION_BATCH when sweeping.
case "${VALUE_BYTES}" in
    64)
        DEFAULT_BANDLE_ADMISSION_WINDOW=512
        DEFAULT_BANDLE_ADMISSION_BATCH=64
        ;;
    128)
        DEFAULT_BANDLE_ADMISSION_WINDOW=384
        DEFAULT_BANDLE_ADMISSION_BATCH=64
        ;;
    256)
        DEFAULT_BANDLE_ADMISSION_WINDOW=384
        ...
        ;;
    512)
        DEFAULT_BANDLE_ADMISSION_WINDOW=320
